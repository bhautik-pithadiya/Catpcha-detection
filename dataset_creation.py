...
def _gen_captcha(img_dir, num_per_image, n, width, height, choices, max_images_count):
    ...
    def generate_permutation(length):
        ...
        while True:
            permutation = ''.join(random.choice(alphanumeric) for _ in range(length))
            yield permutation
    for _ in range(n):
        ...
        for i in range(remain_count):
            permutation_generator = generate_permutation(num_per_image)
            captcha = ''.join(next(permutation_generator))
            thread = threading.Thread(target=generate_captcha_image, args=(captcha,))
